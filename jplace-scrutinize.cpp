...
int main( int argc, char** argv )
{
    ...
    for (auto& sample : sampleset) {
        for( auto& pq : sample ) {
            ...
            if (z_score > 2) { discared_stddev_2++; }
            if (z_score > 3) { discared_stddev_3++; }
            if (z_score > 4) { discared_stddev_4++; }
            if (z_score > 5) { discared_stddev_5++; blame[ edge_num ].for_stddev_5++; }
        }
    }
    ...
    std::cout   << "\t2" << " sigma from the mean:\t" << std::setprecision(3)
                << (discared_stddev_2/static_cast<double>(total_entries))*100 << "%\n";
    std::cout   << "\t3" << " sigma from the mean:\t" << std::setprecision(3)
                << (discared_stddev_3/static_cast<double>(total_entries))*100 << "%\n";
    std::cout   << "\t4" << " sigma from the mean:\t" << std::setprecision(3)
                << (discared_stddev_4/static_cast<double>(total_entries))*100 << "%\n";
    std::cout   << "\t5" << " sigma from the mean:\t" << std::setprecision(3)
